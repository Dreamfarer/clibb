...
class Application():
    ...
    def run(self) -> None:
        ...
        # Change windows
            ...
            if index != -1: self.__add_to_history(index)
    ...
    def add(self, *windows) -> None:
        ...
        for window in windows:
            if isinstance(window, Window): self.__windows.append(window)
    ...
    def remove(self, *windows) -> None:
        ...
        for name in windows:
            if isinstance(name, (str, dict, Window)): 
                ...
                if index == -1: raise Exception(f"Window does not exist")
                del self.__windows[index]
                if self.__window_history[-1] == index or self.__window_history[-1] > len(self.__windows) - 1: 
                    self.__window_history[-1] = len(self.__windows) - 1
            elif isinstance(name, (list, tuple)): self.remove(*name)
            else: raise TypeError("Elements of 'windows' must be of <class 'str'>")
    ...
    def __find_window_index(self, window_or_name: Union[str, dict, Window]) -> int:
        for index, window in enumerate(self.__windows):
            if isinstance(window_or_name, str):
                if window.get_name() == window_or_name: return index
            elif isinstance(window_or_name, dict):
                try:
                    if window.get_name() == window_or_name["name"]: return index
                except: raise Exception(f"Window configuration is lacking 'name' key")
            elif isinstance(window_or_name, Window):
                if window == window_or_name: return index
        return -1
    ...
    def __add_to_history(self, new_index: int) -> None:
        ...
        else: self.__window_history.append(new_index)